...
from cogs.spoilers import SpoilerChannel
...
class Roles:
	...
	async def get_roles_list( self, ctx ):
		...
		for chan, roles in bot.server_configs[ ctx.message.server ][ "hidden_channels" ].items():
	...
	async def update_roles( self, ctx ):		
		for chan_id, r in bot.server_configs[ ctx.message.server ][ 'hidden_channels' ].items():
			roles = [ t[0].id for t in ctx.message.server.get_channel(id).overwrites if t[1].read_messages ]		
			bot.server_configs[ ctx.message.server ][ 'hidden_channels' ][chan_id] = roles
	...
	async def make_channel( self, ctx, channel_name, role_name ):	
		await self.check_config( ctx )
		 try:
            ...
            channel = await self.bot.create_channel(ctx.message.server, name, everyone, ( new_role , can_read ))	
			await bot.add_roles( ctx.message.author, new_role )
			bot.server_configs[ ctx.message.server ][ 'hidden_channels' ][ channel.id ] = [ new_role.id ]
